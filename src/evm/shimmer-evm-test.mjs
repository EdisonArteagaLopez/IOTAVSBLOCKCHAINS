...
async function main() {
  ...
  const AMOUNT_ETH = process.env.EVM_AMOUNT || '0.001'; // SMR units

  if (!PRIVATE_KEY) throw new Error('Falta EVM_PRIVATE_KEY en tu .env');

  const provider = new ethers.JsonRpcProvider(RPC_URL);
  const network = await provider.getNetwork();
  console.log('RPC:', RPC_URL);
  console.log('Network chainId:', Number(network.chainId)); // debe ser 1073 para Shimmer EVM testnet
